...
All notable changes to `befunge93-vault-cracker` will be documented in this file.
...
- fix(ci): hypatia-scan workdir (${{ env.HOME }} resolves empty) (#6)
- fix(ci): hypatia-scan.yml -- pass GITHUB_TOKEN, use --exit-zero (hyperpolymath/hypatia#213) (#3)
- fix(ci): bump erlef/setup-beam SHA for ubuntu24 runner support (#4)
- fix(ci): move secret-scanner Cargo.toml gate from job-level if: to step-level (#5)
