...
of AffineScript). It implements The Elm Architecture (TEA) pattern: a `Model`,
...
JavaScript/JSX, not AffineScript. The Befunge-93 grid displayed is ASCII art with
...
Caveat::
Using React via CDN is not the preferred hyperpolymath approach (AffineScript is
standard) — this is intentional parody of overengineered setups. A production
hyperpolymath app would use AffineScript compiled output.
...
README documents this as intentional parody, but it will confuse AffineScript
